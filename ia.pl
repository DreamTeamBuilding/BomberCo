...
ia(Board, PosIndex, NewPosIndex, TaillePlateau, iav2) :-
    repeat, (posSafe(PosIndex, Board, TaillePlateau) ->
            repeat, Move is random(7),indexAction(Move, I), NewPosIndex is PosIndex+I, posSafe(NewPosIndex, Board, TaillePlateau);
...
ia(Board, PosIndex, NewPosIndex, TaillePlateau, iav3) :-
    repeat, (posSafe(PosIndex, Board, TaillePlateau) ->
            repeat, Move is random(7),indexAction(Move, I), NewPosIndex is PosIndex+I, posSafe(NewPosIndex, Board, TaillePlateau);
            Move is random(5),indexAction(Move, I), NewPosIndex is PosIndex+I), testPosAdjacentes(PosAdjacentesPossibles, PosAdjacentesSafes),
    nth0(NewPosIndex, Board, Elem), Elem==0, !.
